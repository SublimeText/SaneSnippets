...
def parse_snippet(path, text):
	...
	try:
		...
		for key in ['description', 'scope', 'tabTrigger']:
			if not key in keys:
				sublime.error_message('SaneSnippet missing required property "%s" in file "%s"' % (key, path))
				return
	except Exception:
...
def regenerate_snippets():
	...
	# Create SaneSnippets cache dir if it doesn't exist
	snippets_dir = os.path.join(sublime.packages_path(),'User', 'SaneSnippets.cache')
